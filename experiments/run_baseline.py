...
# ⚡ HIGH RECALL UPGRADE 1: Exponential Moving Average to sustain the alarm
def apply_ema_sustain(scores, alpha=0.05):
    ema = np.zeros_like(scores)
    ema[0] = scores[0]
    for i in range(1, len(scores)):
        # If the new score is a massive spike, take it instantly (max). 
        # If it's dropping, let the EMA slowly decay it.
        if scores[i] > ema[i-1]:
            ema[i] = scores[i]
        else:
...
def evaluate_on_test(backbone, test_dataset, cfg, device, val_dataset=None) -> dict:
    ...
    if val_dataset is not None:
        ...
        val_scores = apply_ema_sustain(val_scores) # Apply sustain filter
        smoothed_val = smooth_scores(val_scores, window_size=30) # Wider smooth window
    ...
    test_scores = apply_ema_sustain(test_scores) # Apply sustain filter
    
    best_objective = -1
    best_metrics = None
    best_mult = 4.5 
    
    # Sweep aggressively lower thresholds
    for mult in np.arange(0.5, 5.1, 0.1):
        thresh = float(v_mean + (mult * v_std))
        current_metrics = evaluate(test_scores, test_labels, threshold=thresh, verbose=False)
        
        # ⚡ HIGH RECALL UPGRADE 2: Square the recall to mathematically force a wider net
        objective_score = current_metrics["f1_pa"] * (current_metrics["recall"] ** 2)
