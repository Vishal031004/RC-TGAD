...
class BaseTimeSeriesDataset(Dataset):
    def __init__(self, signals: np.ndarray, labels: np.ndarray,
                 ...
                 graph_threshold: float | None = None): # Allow None for auto-tune
        super().__init__()
...
def build_graph_from_correlation(signals: np.ndarray, threshold: float | None = None) -> Data:
    ...
    # 🛡️ THE FIX: DYNAMIC GRAPH THRESHOLDING
    if threshold is None:
        # Dynamically find a threshold that keeps the strongest ~15% of connections.
        # Clip it so it never goes below 0.15 (too noisy) or above 0.5 (too sparse).
